...
int server(int *pipe2c, int *pipe2m, int size, int n) {
    ...
    char *buffer;
    char mul;
	int port, quickack, servsockfd, connsockfd, addrlen, on, i, one_size, total_size; 
    ...
    for (i=0; i<n; i++) {
        ...
        total_size = 0;
        do {
            one_size = write(pipe2c[1], buffer + total_size, size - total_size);
            total_size += one_size;
        } while (one_size > 0 && total_size < size);
        if (total_size != size) {
            perror("[MAIN] ERROR: Failed to send all data\n"); 
            return 1;
        }
    }
...
int client(int *pipe2c, int *pipe2m, int size, int n) {
    ...
    char mul;
	int port, quickack, sockfd, on, i, one_size, total_size; 
    ...
    for (i=0; i<n; i++) {
        ...
        total_size = 0;
        do {
            one_size = read(pipe2c[0], buffer + total_size, size - total_size);
            total_size += one_size;
        } while (one_size > 0 && total_size < size);
        if (total_size != size) {
            perror("[CHILD] ERROR: Failed to receive all data\n"); 
            return 1;
        }
